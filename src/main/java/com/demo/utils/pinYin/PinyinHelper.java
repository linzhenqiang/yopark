...
public final class PinyinHelper {
	...
	private static final Map<String, String> MUTIL_PINYIN_TABLE = PinyinResource
			.getMutilPinyinResource();
		...
		for (int i = pinyinArray.length - 1; i >= 0; i--) {
			...
			for (int j = originalPinyin.length() - 1; j >= 0; j--) {
				...
				if (originalChar < 'a' || originalChar > 'z') {
					...
					pinyinArray[i] = originalPinyin.replace(String.valueOf(originalChar),
							String.valueOf(replaceChar)) + toneNumber;
					hasMarkedChar = true;
					break;
				}
			}
	...
	private static String[] convertWithoutTone(String pinyinArrayString) {
		...
		for (int i = ALL_MARKED_VOWEL.length() - 1; i >= 0; i--) {
			...
			pinyinArrayString = pinyinArrayString.replace(String.valueOf(originalChar),
	...
	public static String convertToPinyinString(String str, String separator,
			PinyinFormat pinyinFormat) throws PinyinException {
	...
	public static String getShortPinyin(String str) throws PinyinException {
		...
		for (int i = 0, len = str.length(); i < len; i++) {
			...
			if (!ChineseHelper.isChinese(c) && c != CHINESE_LING) {
				...
				// 搜索连续的汉字字符串
				while (j < len && (ChineseHelper.isChinese(str.charAt(j))
						|| str.charAt(j) == CHINESE_LING)) {
	...
	public static void main(String[] args) throws PinyinException {
		String pinyin = "涡河";
		System.out.println(PinyinHelper.getShortPinyin(pinyin));
		System.out.println(PinyinHelper.convertToPinyinString(pinyin, "", PinyinFormat.WITHOUT_TONE));
